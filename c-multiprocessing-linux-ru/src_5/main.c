...
int request_cs(const void * self) {
    ...
    struct request_data data, receive_data;
    ...
    Message *reply;
    for(int i = 1; i < process->process_count; i++)
        if(i != get_process_number(process->pipes, process->process_count)) {
            int quit = 0;
            while(!quit) {
                if(receive(process, i, &tmp) != -1) {
                    set_inc_lamport_time(tmp.s_header.s_local_time);
                    switch(tmp.s_header.s_type) {
                    case CS_REQUEST:
                        receive_data.id   = get_last_sender();
                        receive_data.time = tmp.s_header.s_local_time;
                        // сравнения приоритетов запросов
                        if(more(data, receive_data)) {
                            // если полученный запрос имеет меньший приоритет
                            inc_lamport_time();
                            reply = make_empty_message(CS_REPLY);
                            send(process, get_last_sender(), reply);
                            free_message(reply);
                        } else {
                            // если меньший - откладываем
                            drarray_set(get_last_sender());
                        }
                        break;
                    case CS_REPLY:
                        quit = 1;
...
int release_cs(const void * self) {
    ...
    for(int i = 0; i < process->process_count; i++)
        if(drarray_isset(i)) {
            inc_lamport_time();
            Message *msg = make_empty_message(CS_REPLY);
            send_multicast((void*)process, msg);
            free_message(msg);
            drarray_unset(i);
        }
