...
int*** create_pipes(unsigned int processes) {
    ...
    for(unsigned int i = 0; i < processes; i++) {
        for(unsigned int j = i + 1; j < processes; j++) {
            ...
            int tmp_read[2];
            pipe(tmp_read);
            fcntl(tmp_read[READ_CH], F_SETFL, O_NONBLOCK);
            fcntl(tmp_read[WRITE_CH], F_SETFL, O_NONBLOCK);
            log_pipe_open(j, i, tmp_read[READ_CH], tmp_read[WRITE_CH]);
            // второй для записи в текущий поток
            int tmp_write[2];
            pipe(tmp_write);
            fcntl(tmp_write[READ_CH], F_SETFL, O_NONBLOCK);
            fcntl(tmp_write[WRITE_CH], F_SETFL, O_NONBLOCK);
