...
void add_state(BalanceHistory *history, int balance, int pending, int pending_time) {
    ...
    if(pending > 0) {
        for(int i = pending_time - 1; i < cur_time; i++) {
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < CHILD_PROCESSES_COUNT; i++) {
        if(fork() == 0) {
            ...
            log_events_started(get_lamport_time(), id, balance);
            inc_lamport_time(); ///
            ...
            while(!done) {
                if(receive_any(&process, &in_msg) != -1) {
                    set_inc_lamport_time(in_msg.s_header.s_local_time);
                    ...
                    case TRANSFER:
                        if(get_last_sender() == 0) {
                            ...
                            tmp_amount = order->s_amount;
                            balance -= tmp_amount;
                            add_state(&history, balance, 0, 0);
                            inc_lamport_time(); ///
                            in_msg.s_header.s_local_time
                                                 = get_lamport_time();
                            send(&process, order->s_dst, &in_msg);
                            log_event_transfer_out(get_lamport_time(),
                                                   id, order->s_dst,
                                                   tmp_amount);
                            ...
                            order =(TransferOrder*)in_msg.s_payload;
                            balance += order->s_amount;
                            //add_state(&history, balance, 0);
                            add_state(&history, balance, order->s_amount, in_msg.s_header.s_local_time);
                            log_event_transfer_in(get_lamport_time(),
                                                  order->s_src, id,
                                                  order->s_amount);
                            inc_lamport_time(); ///
                            ack = make_empty_message(ACK);
            ...
            // полезной работы
            log_events_done(get_lamport_time(), id, balance);
            inc_lamport_time(); ///
